...
# Import Data
input_path = "../../../examples/pareto_optimality/input/timeseries_import.csv"
input_data = np.recfromcsv(input_path, encoding=None)
results_path = "../../../examples/pareto_optimality/output/timeseries_export.csv"
...
axarr[0].plot(times, [Vlimits_c[0]]*len(times), label="$V_{min}$ (flood control)",
              linewidth=2, linestyle='--', color="m")
axarr[0].plot(times, [Vlimits_c[1]]*len(times), label="$V_{max}$ (flood control)",
              linewidth=2, linestyle='--', color="m")
axarr[0].plot(times, [Vlimits_b[0]]*len(times), label="$V_{min}$ (boating)",
              linewidth=2, linestyle=':', color="g")
axarr[0].plot(times, [Vlimits_b[1]]*len(times), label="$V_{max}$ (boating)",
              linewidth=2, linestyle=':', color="g")
axarr[0].plot(times, results["troutlake_v"], label="$V$ (reservoir)", linewidth=2,
              marker='.', color="b")
...
axarr[1].plot(times, [Qlimits[0]]*len(times), label="$Q_{min}$ (ecology)", linestyle='--',
              linewidth=2, color="m")
axarr[1].plot(times, [Qlimits[1]]*len(times), label="$Q_{max}$ (ecology)", linestyle='--',
              linewidth=2, color="m")
axarr[1].plot(times, input_data["rafting_qmin"], label="$Q_{min}$ (rafting)", linestyle=':',
              linewidth=2, color="g")
axarr[1].plot(times, input_data["rafting_qmax"], label="$Q_{max}$ (rafting)", linestyle=':',
              linewidth=2, color="g")
axarr[1].plot(times, results["rivercity_q"], label="$Q$ (River City)", linewidth=2, marker='.',
              color="orange")
...
for i in range(n_subplots):
    ...
    axarr[i].legend([handles[idx] for idx in legend_order],

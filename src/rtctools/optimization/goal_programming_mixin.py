        ...
        for i_goal in range(len(goals)):
            for ensemble_member in range(self.ensemble_size):
                ...
                if old_key in self.__results[ensemble_member]:
                    ...
                    self.__results[ensemble_member][new_key] = self.__results[ensemble_member][old_key]
            ...
            if i == 0:
                ...
                for goal in itertools.chain(goals, path_goals):
                    if not goal.critical and not goal.has_target_bounds:
                        skip_solve = False
                        break
                    pre_solve_vals = [self.__results[ensemble][goal.function_key] for ensemble in range(self.ensemble_size)]

...
class CSVLookupTableProvider:
    """Backend parent class for Simulation and Optimization CSVLookupTableMIxin
    ...
    * curvature:
    ...
    def pre(self):
        ...
        try:
            ...
            logger.info(
                "CSVLookupTableMixin: No curvefit_options.ini file found. "
        ...
        def get_curvefit_options(curve_name, no_curvefit_options=no_curvefit_options):
            ...
            def get_property(prop_name):
                try:
                    ...
                    raise Exception(
                        "CSVLookupTableMixin: Invalid {0} constraint for {1}. {0} "
                        "should be either -1, 0, or 1.".format(prop_name, curve_name)
                    )
                return prop
        ...
        for filename in glob.glob(os.path.join(self.__lookup_table_folder, "*.csv")):
            ...
            logger.debug(
                "CSVLookupTableMixin: Output is {}, inputs are {}.".format(
                    output, inputs
                )
            )

            tck = None
            # If tck file is newer than the csv file, first try
            # to load the cached values from the tck file
            tck_filename = filename.replace(".csv", ".tck")
            valid_cache = False
            if os.path.exists(tck_filename):
                if no_curvefit_options:
                    valid_cache = os.path.getmtime(filename) < os.path.getmtime(
                        tck_filename
                    )
                else:
                    valid_cache = (
                ...
                if valid_cache:
                    logger.debug(
                        "CSVLookupTableMixin: Attempting to use cached tck "
                        "values for {}".format(output)
                    )
            ...
            if not valid_cache:
                logger.info(
                    "CSVLookupTableMixin: Recalculating tck values for {}".format(
                        output
                    )
                )
            ...
            if len(csvinput.dtype.names) == 2:
                if not valid_cache:
                    ...
                    if len(csvinput[output]) >= k + 1:
                        ...
                        raise Exception(
                            "CSVLookupTableMixin: Too few data points in {} to"
                            "do spline fitting. Need at least {} points.".format(
                                filename, k + 1
                            )
                        )
                ...
                self._provided_lookup_tables[output] = LookupTable(
                    symbols, function, tck
                )
                ...
                function = ca.Function(
                    "f", symbols, [BSpline2D(*tck)(symbols[0], symbols[1])]
                )
                self._provided_lookup_tables[output] = LookupTable(
                    symbols, function, tck
                )
                ...
                raise Exception(
                    "CSVLookupTableMixin: {}-dimensional lookup tables not "
                    "implemented yet.".format(len(csvinput.dtype.names))
                )

...
class GoalProgrammingMixin(OptimizationProblem, metaclass=ABCMeta):
    ...
    def __min_max_arrays(self, g, target_shape=None):
        ...
        if isinstance(g.target_min, Timeseries):
            ...
            m = np.broadcast_to(g.target_min, (target_shape, len(g.target_min))).transpose()
        elif target_shape:
        ...
        if isinstance(g.target_max, Timeseries):
            ...
            M = np.broadcast_to(g.target_max, (target_shape, len(g.target_max))).transpose()
        elif target_shape:
    ...
    def __validate_goals(self, goals, is_path_goal):
        ...
        for goal in goals:
            ...
            if goal.has_target_min:
                ...
                if np.any(goal_m[indices] <= lb[indices]):
                    raise Exception(
            ...
            if goal.has_target_max:
                ...
                if np.any(goal_M[indices] >= ub[indices]):
                    raise Exception(
    ...
    def optimize(self, preprocessing=True, postprocessing=True, log_solver_failure_as_error=True):
        ...
        if isinstance(options['obj_constr_relaxation'], (float, int)):
            obj_cons_rel = options['obj_constr_relaxation']
            options['obj_constr_relaxation'] = [obj_cons_rel] * (self.__cardinality_priorities - 1)
        if len(options['obj_constr_relaxation']) > 0:

...
class ModelPathConstraintsTimeseriesVector(ModelPathConstraintsTimeseries):
    ...
    def path_constraints(self, ensemble_member):
        ...
        m = np.vstack((x_min, z_min))
        M = np.vstack((x_max, z_max))
...
class ModelConstantInputsVector(Model):
    ...
    def constant_inputs(self, ensemble_member):
        ...
        constant_inputs[self._input_var.name()] = \
            Timeseries(self.times(), np.vstack((1 * np.arange(n_times),
                                                2 * np.arange(n_times),
                                                3 * np.arange(n_times))))
        return constant_inputs
...
class TestVectorConstraints(TestCase):
    ...
    def test_vector_parameters(self):
        ...
        v1 = np.vstack(tuple(results1[p.name()] for p in self.problem1._additional_vars))
    ...
    def test_additional_path_variables(self):
        ...
        ref = np.vstack((results1['u'], results1['u']**2, results1['u']**3))

        v1 = np.vstack(tuple(results1[p.name()] for p in self.problem1._additional_path_vars))
    ...
    def test_additional_path_variables_state_at(self):
        ...
        v1 = np.vstack(tuple(results1[p.name()] for p in self.problem1._additional_path_vars))
        v2 = np.vstack(tuple(results2[p.name()] for p in self.problem2._additional_path_vars))

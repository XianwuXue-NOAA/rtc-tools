...
class Plotting:
    ...
    def plot_goal_results_from_self(self, priority=None):
        self.plot_goals_results(self.timeseries_import.times,
    ...
    def plot_goals_results(self, timeseries_import_times, extract_result, range_goals, min_q_goals, priority=None, result_dict=None):
        t = self.times()
        ...
        def apply_general_settings():
            ...
            positive_dict = {name.replace(".","_"): value for name, value in result_dict["positive_effect_dict"].items()}
            negative_dict = {name.replace(".","_"): value for name, value in result_dict["negative_effect_dict"].items()}
            if goal_variable in positive_dict:
                ...
                for xr in positive_dict[goal_variable]:
                    ...
                    axs[i_r, i_c].plot(t_datetime[int(xr)], results[goal_variable][int(xr)], marker=matplotlib.markers.CARETUPBASE, color='r', label=label)
            if goal_variable in negative_dict:
                for xr in negative_dict[goal_variable]:
                    ...
                    # else:
                    #     existing_labels.append(label)
                    axs[i_r, i_c].plot(t_datetime[int(xr)], results[goal_variable][int(xr)], marker=matplotlib.markers.CARETDOWNBASE, color='b', label=label)
                    # axs[i_r, i_c].vlines(x=t_datetime[int(xr)], color='r', linestyle ='--',ymin= axs[i_r, i_c].get_ylim()[0],ymax= axs[i_r, i_c].get_ylim()[1])
            return i_c, i_r
    ...
    def priority_completed(self, priority: int) -> None:
        ...
        to_store = {"extract_result": self.extract_results(),

...
class GoalProgrammingMixin(OptimizationProblem, metaclass=ABCMeta):
    ...
    def optimize(self, preprocessing=True, postprocessing=True, log_solver_failure_as_error=True):
        ...
        if _log_int_cast_error:
            raise Exception(
                "GoalProgrammingMixin: All goal priorities must be of type int or castable to int for goal {}".format(
                    _log_int_cast_error))
        ...
        if _log_int_cast_error:
            raise Exception(
                "GoalProgrammingMixin: All path goal priorities must be of type int or castable to int for goal {}".format(
                    _log_int_cast_error))
